 ...
 */
@PublicEvolving
public final class SqlDecimal implements Comparable<SqlDecimal>, Serializable {
	...
	private SqlDecimal(int precision, int scale, long longVal, BigDecimal decimalVal) {
	...
	@Override
	public int compareTo(SqlDecimal that) {
	...
	public static SqlDecimal fromLong(long l, int precision, int scale) {
		checkArgument(precision > 0 && precision <= MAX_LONG_DIGITS);
		checkArgument((l >= 0 ? l : -l) < POW10[precision]);
		return new SqlDecimal(precision, scale, l, null);
	}
	...
	public static SqlDecimal fromBigDecimal(BigDecimal bd, int precision, int scale) {
		...
		return new SqlDecimal(precision, scale, longVal, bd);
	}

	public static SqlDecimal zero(int precision, int scale) {
		if (precision <= MAX_COMPACT_PRECISION) {
			return new SqlDecimal(precision, scale, 0, null);
		} else {
			return fromBigDecimal(BigDecimal.ZERO, precision, scale);
		}
	}

	public SqlDecimal copy() {
		return new SqlDecimal(precision, scale, longVal, decimalVal);
	}
	...
	public static SqlDecimal fromUnscaledLong(int precision, int scale, long longVal) {
		assert isCompact(precision);
		return new SqlDecimal(precision, scale, longVal, null);
	}
	...
	public static SqlDecimal fromUnscaledBytes(int precision, int scale, byte[] bytes) {
		if (precision > MAX_COMPACT_PRECISION) {
			BigDecimal bd = new BigDecimal(new BigInteger(bytes), scale);
			return new SqlDecimal(precision, scale, -1, bd);
		}
	...
	public SqlDecimal negate() {
		if (isCompact()) {
			return new SqlDecimal(precision, scale, -longVal, null);
		} else {
			return new SqlDecimal(precision, scale, -1, decimalVal.negate());
		}
	}

	public SqlDecimal abs() {
		if (isCompact()) {
			if (longVal >= 0) {
				return this;
			} else {
	...
	public static SqlDecimal add(SqlDecimal v1, SqlDecimal v2, int precision, int scale) {
		if (v1.isCompact() && v2.isCompact() && v1.scale == v2.scale) {
			...
			try {
				...
				return new SqlDecimal(precision, scale, ls, null);
	...
	public static SqlDecimal subtract(SqlDecimal v1, SqlDecimal v2, int precision, int scale) {
		if (v1.isCompact() && v2.isCompact() && v1.scale == v2.scale) {
			assert scale == v1.scale; // no need to rescale
			try {
	...
	public static SqlDecimal multiply(SqlDecimal v1, SqlDecimal v2, int precision, int scale) {
		BigDecimal bd = v1.toBigDecimal().multiply(v2.toBigDecimal());
		return fromBigDecimal(bd, precision, scale);
	}

	public static SqlDecimal divide(SqlDecimal v1, SqlDecimal v2, int precision, int scale) {
		BigDecimal bd = v1.toBigDecimal().divide(v2.toBigDecimal(), MC_DIVIDE);
		return fromBigDecimal(bd, precision, scale);
	}

	public static SqlDecimal mod(SqlDecimal v1, SqlDecimal v2, int precision, int scale) {
	 ...
	 */
	public static SqlDecimal divideToIntegralValue(SqlDecimal value, SqlDecimal divisor, int precision, int scale) {
	...
	public static long castToIntegral(SqlDecimal dec) {
	...
	public static long castToLong(SqlDecimal dec) {
		return castToIntegral(dec);
	}

	public static int castToInt(SqlDecimal dec) {
		return (int) castToIntegral(dec);
	}

	public static short castToShort(SqlDecimal dec) {
		return (short) castToIntegral(dec);
	}

	public static byte castToByte(SqlDecimal dec) {
		return (byte) castToIntegral(dec);
	}

	public static float castToFloat(SqlDecimal dec) {
		return (float) dec.doubleValue();
	}

	public static double castToDouble(SqlDecimal dec) {
		return dec.doubleValue();
	}

	public static SqlDecimal castToDecimal(SqlDecimal dec, int precision, int scale) {
		return fromBigDecimal(dec.toBigDecimal(), precision, scale);
	}

	public static boolean castToBoolean(SqlDecimal dec) {
		return dec.toBigDecimal().compareTo(BigDecimal.ZERO) != 0;
	}

	public static long castToTimestamp(SqlDecimal dec) {
		return (long) (dec.doubleValue() * 1000);
	}

	public static SqlDecimal castFrom(SqlDecimal dec, int precision, int scale) {
		return fromBigDecimal(dec.toBigDecimal(), precision, scale);
	}

	public static SqlDecimal castFrom(String string, int precision, int scale) {
		return fromBigDecimal(new BigDecimal(string), precision, scale);
	}

	public static SqlDecimal castFrom(double val, int p, int s) {
		return fromBigDecimal(BigDecimal.valueOf(val), p, s);
	}

	public static SqlDecimal castFrom(long val, int p, int s) {
		return fromBigDecimal(BigDecimal.valueOf(val), p, s);
	}

	public static SqlDecimal castFrom(boolean val, int p, int s) {
	 ...
	 */
	public static SqlDecimal sign(SqlDecimal b0) {
		if (b0.isCompact()) {
			return new SqlDecimal(b0.precision, b0.scale, b0.signum() * POW10[b0.scale], null);
		} else {
			return fromBigDecimal(BigDecimal.valueOf(b0.signum()), b0.precision, b0.scale);
		}
	}

	public static int compare(SqlDecimal b1, SqlDecimal b2){
		return b1.compareTo(b2);
	}

	public static int compare(SqlDecimal b1, long n2) {
		if (!b1.isCompact()) {
	...
	public static int compare(SqlDecimal b1, double n2) {
		return Double.compare(b1.doubleValue(), n2);
	}

	public static int compare(long n1, SqlDecimal b2) {
		return -compare(b2, n1);
	}

	public static int compare(double n1, SqlDecimal b2) {
		return -compare(b2, n1);
	}
	 ...
	 */
	public static SqlDecimal sround(SqlDecimal b0, int r) {
		if (r >= b0.scale) {
			return b0;
		}
	...
	static SqlDecimal readDecimalFieldFromSegments(MemorySegment[] segments, int baseOffset,
