...
import org.apache.flink.table.factories.TestFormatFactory.FormatReaderProviderMock;
import org.apache.flink.table.factories.TestFormatFactory.FormatWriterProviderMock;
...
public class FactoryUtilTest {
	...
	public void testAllOptions() {
		...
		final DynamicTableSource expectedSource = new DynamicTableSourceMock(
			"MyTarget",
			new FormatReaderProviderMock(",", false),
			new FormatReaderProviderMock("|", true));
		assertEquals(expectedSource, actualSource);
		final DynamicTableSink actualSink = createTableSink(options);
		final DynamicTableSink expectedSink = new DynamicTableSinkMock(
			"MyTarget",
			1000L,
			new FormatWriterProviderMock(","),
			new FormatWriterProviderMock("|"));
