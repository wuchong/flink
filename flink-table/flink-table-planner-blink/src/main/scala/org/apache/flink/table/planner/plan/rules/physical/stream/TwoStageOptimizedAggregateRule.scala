...
import org.apache.flink.table.planner.plan.`trait`.{ChangelogModeTrait, FlinkRelDistribution, FlinkRelDistributionTraitDef}
...
import org.apache.flink.table.planner.plan.utils.ChangelogPlanUtils.INSERT_ONLY_MODE
...
class TwoStageOptimizedAggregateRule extends RelOptRule(
  ...
  override def matches(call: RelOptRuleCall): Boolean = {
    ...
    val needRetraction = ChangelogPlanUtils.containsRetraction(realInput)
  ...
  private def createTwoStageAgg(
    ...
    // local agg shouldn't produce update/delete messages
    val localAggTraitSet = input.getTraitSet.plus(new ChangelogModeTrait(Some(INSERT_ONLY_MODE)))
