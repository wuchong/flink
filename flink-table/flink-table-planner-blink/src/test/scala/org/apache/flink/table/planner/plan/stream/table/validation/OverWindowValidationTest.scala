...
class OverWindowValidationTest extends TableTestBase {
  ...
  def testInvalidWindowAlias(): Unit = {
    ...
    optimize(TableTestUtil.toRelNode(result), requestUpdateBefore = true)
  }
  ...
  private def optimize(rel: RelNode, requestUpdateBefore: Boolean = true): RelNode = {
    val newRel = if (requestUpdateBefore) {
      rel.copy(
        rel.getTraitSet.plus(new SendBeforeImageForUpdatesTrait(true)),
        rel.getInputs)
    } else {
      rel
    }
    val planner = streamUtil.tableEnv.asInstanceOf[TableEnvironmentImpl].getPlanner
    planner.asInstanceOf[PlannerBase].optimize(Seq(newRel)).head
  }
}
