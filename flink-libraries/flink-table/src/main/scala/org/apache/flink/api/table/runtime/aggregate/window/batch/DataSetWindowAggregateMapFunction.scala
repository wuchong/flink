...
/** This map function only works for windows on batch tables. The differences between this function
  * and [[org.apache.flink.api.table.runtime.aggregate.AggregateMapFunction]] is this function
  * append a rowtime field to the end of the output row.
  */
class DataSetWindowAggregateMapFunction(
  ...
  override def open(config: Configuration) {
    ...
    val partialRowLength = groupingKeys.length +
      aggregates.map(_.intermediateDataType.length).sum + 1  // additional one field is for rowtime
  ...
  override def map(input: Row): Row = {
    ...
    val timeField = input.productElement(timeFieldPos)
