...
object AggregateUtil {
  ...
  private[flink] def createGroupAggregateFunction(
    ...
    val genFunction = generator.generateAggregations(
      ...
      needMerge = false,
      needReset = false,
      accConfig = Some(LazyAccumulatorConfig(accSpecs, isOnState = true)))

    val accConfig = accSpecs.flatMap(specs => specs.map(s => (s.id, s.toStateDescriptor)))
  ...
  : (Array[Array[Int]], Array[TableAggregateFunction[_, _]], Array[TypeInformation[_]], Array[Seq[AccumulatorSpec[_]]]) = {
    ...
    val accSpecs = new Array[Seq[AccumulatorSpec[_]]](aggregateCalls.size)
