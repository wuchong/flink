...
public class HBaseConnectorITCase extends HBaseTestBase {
	...
	@Test
	public void testTableSourceSink() throws Exception {
		StreamExecutionEnvironment execEnv = StreamExecutionEnvironment.getExecutionEnvironment();
		StreamTableEnvironment tEnv = StreamTableEnvironment.create(execEnv, streamSettings);

		// register HBase table testTable1 and testTable2 which contains test data
		tEnv.executeSql(createHBaseTableDDL(TEST_TABLE_1, false));
		tEnv.executeSql(createHBaseTableDDL(TEST_TABLE_2, false));
		...
		batchEnv.executeSql(createHBaseTableDDL(TEST_TABLE_2, false));
	...
	@Test
	public void testTableSourceSinkWithTimeTypes() throws Exception {
		// only test TIMESTAMP/DATE/TIME/DECIMAL for new connector (using blink-planner),
		// because new connector encodes DATE/TIME to int, the old one encodes to long,
		// and DECIMAL with precision works well in new connector.
	...
	public void testHBaseLookupTableSource() throws Exception {
		if (OLD_PLANNER.equals(planner) || isLegacyConnector) {
			...
			// skip for old planner and old connector
			return;
		}
