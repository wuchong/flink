...
public class HBaseSerde {
	...
	private GenericRowData rowKeyWrapper = new GenericRowData(1);
	 ...
	 */
	public @Nullable Get createGet(Object rowKey) {
		checkArgument(keyEncoder != null, "row key is not set.");
		rowKeyWrapper.setField(0, rowKey);
		byte[] rowkey = keyEncoder.encode(rowKeyWrapper, 0);
		if (rowkey.length == 0) {
			// drop dirty records, rowkey shouldn't be zero length
			return null;
		}
